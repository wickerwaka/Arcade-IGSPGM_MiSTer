...
| 00-C | host osc IRQ behavior **RESOLVED (2026-06-16, IRQ-counter probe on a stopped voice — detection via the Z80 osc-IRQ counter; IRQV/0x4B are drained by the Z80 handler so they read idle)**: the discriminator is **bit5 (IRQ-enable), NOT bit7 (the "pend" status bit)**. osc_conf 0x80 (bit7 only) → 0 IRQs; 0xA0 (bit7+bit5) → IRQ fires; **0x20 (bit5 only, no bit7) → IRQ also fires** (count fixed at 2 over 50–500 ms, not a storm). So bit7 is NOT a host-settable pend trigger — the osc INT is a *level* of (end/done condition AND bit5 enable); the old "host pend = bit7+bit5, bit7 alone no-op" framing was misleading. RTL fix (`ics2115.sv:1217`: host pend gated on bit7 AND bit5) matches the 0x80/0xA0 host-write observables and removes the original bit7-alone spurious pend. **Known gap:** the RTL latches the pend at the engine end-event (edge), so enabling bit5 *after* a voice is already done (the 0x20 case) won't surface the level INT — academic for real games (they enable IRQ at/before key-on) | T-IRQV | TESTED-PASS |
